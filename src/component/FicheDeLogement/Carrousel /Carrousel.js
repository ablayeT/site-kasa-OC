...
import "./style/_Carrousel.scss";

function Carrousel({ slidesPictures }) {
  const [current, setCurrent] = useState(0); // je définis l'index du premier slide à 0
  const length = slidesPictures ? slidesPictures.length : 0; // longueur du tableau de slides
  console.log("slidePictures: ", slidesPictures);
  ...
  return (
    <div id="carrousel-container">
      ...
      {slidesPictures && slidesPictures.map((slide, index) => (
        <div
